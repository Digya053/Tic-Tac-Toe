...
class CheckTicTacToe():
	...
	def check_board_size(self):
			while int(self.board_size) % 2 == 0:
					self.board_size = input("Please enter an odd number for a board size: ")
	...
	def check_diagonal(self):
			...
			if middle_number != 0:
					...
					while change_column_up != self.board_size - 1 and change_row_down != self.board_size - 1:
							...
							if self.final_board[change_row_up][change_column_up] == middle_number:
									diagonal_elements_second.append(self.final_board[change_row_up][change_column_up])
							if self.final_board[change_row_down][change_column_down] == middle_number:
									diagonal_elements_second.append(self.final_board[change_row_down][change_column_down])
