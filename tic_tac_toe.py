...
class PlayTicTacToe():
		...
		def adjust_players(self):
				try:
					...
					if player_user != 1 and player_user != 2:
							player_user = int(input("You can only choose '1' or '2' as a player. Please choose again:  "))
				except ValueError:
					player_user = int(input("You can only choose '1' or '2' as a player. Please choose again: "))
		...
		def display_remaining_places(self):
				...
				time.sleep(1)
				print("\nThere are " + str(len(self.remaining_places)) + " remaining places to place entry.")
		...
		def update_entries(self):
				...
				row = int(input('Enter the row number in which you want to place your entry: '))
				column = int(input('Enter the column number in which you want to place your entry: '))
...
if __name__ == "__main__":
		...
		while length != 0:
			...
			if len(play_tic_tac_toe.occupied_places) >= int(board_size):
				check_winner = CheckTicTacToe(final_board = play_tic_tac_toe.initial_board)
				check_winner.check_all()
				if check_winner.found_winner == True:
						print("Congratulations! " + play_tic_tac_toe.switch_player + " is the winner")
						break
